...
function browser(host,artifacts,label){const root=$(host),items=playable(artifacts);root.replaceChildren();if(!items.length){const stage=el('div','media-stage');stage.append(el('div','empty-media','No playable recording is available. Use the collapsed artifact manifest for available evidence.'));root.append(stage);return}let i=Math.max(0,items.findIndex(x=>x.kind==='video'));if(i<0)i=0;const stage=el('div','media-stage'),controls=el('div','media-controls'),prev=el('button','', '← Previous'),count=el('span',''),next=el('button','', 'Next →'),strip=el('div','filmstrip');controls.append(prev,count,next);function render(){stage.replaceChildren();const a=items[i],role=a.metadata?.camera_role,n=document.createElement(a.kind==='video'?'video':'img');n.src=url(a);n.controls=a.kind==='video';n.alt=`${label} ${role||'recording'} ${i+1}`;stage.append(n);count.textContent=`${role?pretty(role)+' camera · ':''}Recording ${i+1} of ${items.length}`;prev.disabled=i===0;next.disabled=i===items.length-1;strip.replaceChildren();items.forEach((a,j)=>{const b=el('button',j===i?'active':''),role=a.metadata?.camera_role;b.type='button';b.setAttribute('aria-label',`Select ${label} ${role||'recording'} ${j+1}`);b.onclick=()=>{i=j;render()};if(a.kind==='video')b.textContent=`▶ ${role?pretty(role):'Video'}`;else{const img=document.createElement('img');img.src=url(a);img.alt=role?`${pretty(role)} camera`:'';b.append(img)}strip.append(b)})}prev.onclick=()=>{if(i){i--;render()}};next.onclick=()=>{if(i<items.length-1){i++;render()}};root.tabIndex=0;root.onkeydown=e=>{if(e.key==='ArrowLeft')prev.click();if(e.key==='ArrowRight')next.click()};render();root.append(stage,controls,strip)}
