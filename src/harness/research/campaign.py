"""Durable scientific campaign state for the Agents SDK runtime."""
...
class ResearchCampaignStore:
    """SQLite scientific memory; trajectory and media blobs stay on disk."""
        ...
        with closing(self._connect()) as connection:
            connection.executescript("""
                CREATE TABLE IF NOT EXISTS research_campaigns (
                    campaign_id TEXT PRIMARY KEY, objective TEXT NOT NULL,
                    ...
                    model_provider TEXT, model_name TEXT, capability_version TEXT,
                    simulator_metadata_json TEXT NOT NULL DEFAULT '{}',
                    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
                );
                CREATE TABLE IF NOT EXISTS operator_instructions (
                    instruction_id TEXT PRIMARY KEY, campaign_id TEXT NOT NULL,
                    instruction TEXT NOT NULL, consumed INTEGER NOT NULL DEFAULT 0,
                    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                ...
                CREATE TABLE IF NOT EXISTS research_iterations (
                    ...
                    context_json TEXT NOT NULL DEFAULT '{}', isaac_run_id TEXT,
                    reactor_run_id TEXT, plan_c_pair_id TEXT, comparison_status TEXT,
                    human_review_state TEXT, error TEXT,
                    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
                ...
                CREATE TABLE IF NOT EXISTS campaign_events (
                    event_id TEXT PRIMARY KEY, campaign_id TEXT NOT NULL,
            ...
            for name in ("reactor_run_id", "plan_c_pair_id", "comparison_status", "human_review_state"):
                self._ensure_column(connection, "research_iterations", name, "TEXT")
            connection.commit()
    ...
    def create_campaign(
        self, objective: str, *, experiment_budget: int,
        constraints: dict | None = None, model_provider: str | None = None,
        model_name: str | None = None, capability_version: str | None = None,
        simulator_metadata: dict | None = None,
    ) -> str:
        ...
        with closing(self._connect()) as connection:
            connection.execute(
                "INSERT INTO research_campaigns(campaign_id, objective, constraints_json, status, "
                "experiment_budget, model_provider, model_name, capability_version, simulator_metadata_json) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (campaign_id, objective.strip(), json.dumps(constraints or {}, sort_keys=True),
                 CampaignState.CREATED, experiment_budget, model_provider, model_name,
                 capability_version, json.dumps(simulator_metadata or {}, sort_keys=True)),
            )
    ...
    def transition_campaign(self, campaign_id: str, state: CampaignState | str, *, reason: str | None = None) -> None:
        ...
        with closing(self._connect()) as connection:
            updated = connection.execute("UPDATE research_campaigns SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE campaign_id = ?", (state, campaign_id))
    ...
    def begin_iteration(self, campaign_id: str, context: dict) -> str:
        ...
        with closing(self._connect()) as connection:
            ...
            self._event(connection, campaign_id, "iteration_started", {"iteration_id": iteration_id})
            connection.commit()
        return iteration_id

    def transition_iteration(self, iteration_id: str, state: IterationState | str, *, error: str | None = None) -> None:
        ...
        with closing(self._connect()) as connection:
            ...
            connection.execute("UPDATE research_iterations SET state = ?, error = COALESCE(?, error), updated_at = CURRENT_TIMESTAMP WHERE iteration_id = ?", (state, error, iteration_id))
    ...
    def latest_iteration(self, campaign_id: str) -> dict | None:
        with closing(self._connect()) as connection:
            row = connection.execute("SELECT * FROM research_iterations WHERE campaign_id = ? ORDER BY ordinal DESC LIMIT 1", (campaign_id,)).fetchone()
        return self._iteration_row(row) if row else None
    ...
    def record_plan_c_pair(self, campaign_id: str, isaac_run_id: str, pair_id: str) -> str:
        with closing(self._connect()) as connection:
            iteration = connection.execute("SELECT iteration_id, plan_c_pair_id FROM research_iterations WHERE campaign_id = ? AND isaac_run_id = ? ORDER BY ordinal DESC LIMIT 1", (campaign_id, isaac_run_id)).fetchone()
            if iteration is None:
            ...
            connection.execute("UPDATE research_iterations SET plan_c_pair_id = ?, state = ?, updated_at = CURRENT_TIMESTAMP WHERE iteration_id = ?", (pair_id, IterationState.WAITING_FOR_ASSESSMENT, iteration["iteration_id"]))
            self._event(connection, campaign_id, "reactor_pair_prepared", {"iteration_id": iteration["iteration_id"], "run_id": isaac_run_id, "pair_id": pair_id})
            connection.commit()
        return iteration["iteration_id"]

    def record_pair_comparison(self, campaign_id: str, pair_id: str, reactor_run_id: str, comparison_status: str) -> None:
        with closing(self._connect()) as connection:
            iteration = connection.execute("SELECT iteration_id FROM research_iterations WHERE campaign_id = ? AND plan_c_pair_id = ?", (campaign_id, pair_id)).fetchone()
            if iteration is None:
                raise KeyError("the pair does not belong to this campaign")
            connection.execute("UPDATE research_iterations SET reactor_run_id = ?, comparison_status = ?, state = ?, updated_at = CURRENT_TIMESTAMP WHERE iteration_id = ?", (reactor_run_id, comparison_status, IterationState.RECORDED, iteration["iteration_id"]))
            self._event(connection, campaign_id, "plan_c_comparison_recorded", {"iteration_id": iteration["iteration_id"], "pair_id": pair_id, "reactor_run_id": reactor_run_id, "comparison_status": comparison_status})
            connection.commit()

    def list_events(self, campaign_id: str) -> list[dict]:
        with closing(self._connect()) as connection:
            rows = connection.execute("SELECT * FROM campaign_events WHERE campaign_id = ? ORDER BY rowid", (campaign_id,)).fetchall()
    ...
    def record_event(self, campaign_id: str, event_type: str, payload: dict | None = None) -> str:
        ...
        event_id = str(uuid4())
        with closing(self._connect()) as connection:
            if connection.execute("SELECT 1 FROM research_campaigns WHERE campaign_id = ?", (campaign_id,)).fetchone() is None:
                raise KeyError(f"unknown campaign: {campaign_id}")
            connection.execute("INSERT INTO campaign_events(event_id, campaign_id, event_type, payload_json) VALUES (?, ?, ?, ?)", (event_id, campaign_id, event_type.strip(), json.dumps(payload or {}, sort_keys=True)))
            connection.commit()
        return event_id
    ...
    def _campaign_row(row: sqlite3.Row) -> dict:
        ...
        result.pop("last_openai_response_id", None)
        return result

    @staticmethod
    def _iteration_row(row: sqlite3.Row) -> dict:
        result = dict(row)
        result["context"] = json.loads(result.pop("context_json", "{}") or "{}")
        for obsolete in ("proposal_json", "compiled_json", "openai_response_id"):
